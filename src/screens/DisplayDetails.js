...
const DisplayDetails = ({ navigation }) => {
    ...
    const getData = async () => {
        try {
            ...
            //if (!datastring || datastring === "null") {//ikkada async lo data lekapothene api call avthundhi oho ok
    ...
    const navigateToForm = (item) => {
        console.log(item);
        navigation.navigate('SaleForm', { selectedItem: item });
        ...
        return (
            <ScrollView>
                {data.map((item) => (
                    ...
                    >
                        <Text style={styles.cardText}>Shop Name: {item.shopname}</Text>
                        <Text style={styles.cardText}>Shop Owner: {item.shopowner}</Text>
